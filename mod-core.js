(() => {
  ...
  // và gán vào MOD.BUTTON_GROUPS trước khi gọi MOD.init().
  // =========================================================
  ...
  const MOD = {
    ...
    // Sẽ được mod-buttons.js gán đè trước khi init() chạy
    BUTTON_GROUPS: [],
    ALLOWED_URL_MATCH: "/app.v2.php",
    ...
    init() {
      ...
      if (!Array.isArray(this.BUTTON_GROUPS) || this.BUTTON_GROUPS.length === 0) {
        console.warn(
          "[MOD] BUTTON_GROUPS rỗng - hãy nạp mod-buttons.js trước khi gọi MOD.init()",
        );
      }
    ...
    createTab() {
      ...
      const groupsHtml = this.BUTTON_GROUPS.map((group) =>
    ...
    bindEvents() {
      ...
      this.BUTTON_GROUPS.forEach((group) => {
  ...
  // =========================================================
  // GLOBAL
  // KHÔNG gọi MOD.init() ở đây.
  // mod-loader.js sẽ gọi MOD.init() sau khi đã nạp mod-buttons.js.
  // =========================================================
